...
fn main() {
    ...
    if matches.is_present("example") {
        ...
        table.add_row(row!["Name", "Internal Address", "Allowed IPs"]);
        ...
        if matches.is_present("linux-script") {
            ...
            configuration
                ...
                .for_each(|allowed_ip| println!("ip route add {} dev wg0", allowed_ip));
        }
        ...
        if matches.is_present("linux-script") {
            println!("EOF");
            println!("ip link add dev wg0 type wireguard");
            println!("ip address add dev wg0 {}/32", client.internal_address());
            println!("wg setconf wg0 vpn.conf");
            println!("ip link set up dev wg0");
            ...
            println!(
                "sudo ifconfig utun9 inet {} 255.255.255.255 {}",
            ...
            configuration
                ...
                .for_each(|allowed_ip| {
                    if allowed_ip.prefix_len() == 32 {
                        println!("sudo route add {} -interface utun9", allowed_ip.addr());
                    } else {
                        println!("sudo route add -net {} -interface utun9", allowed_ip);
                    }
                });
        }
    } else {
